...
function caltotal(gcost)
{
	...
	if(gcost == 'ebook'){
		...
		if($("#egroup").prop("checked") == true){
                ebookcost = parseFloat($('#ebookcost').val());
				tcost = tcost + ebookcost;
            }
            else if($("#egroup").prop("checked") == false){
				if(tcost > 0){
                	ebookcost = parseFloat($('#ebookcost').val());
					tcost = tcost - ebookcost;
				}
...
#toptitle span { text-transform:uppercase;color:#ffa57d; }
...
<input name="selectsecrets" type="hidden" id="secretscheckbox" value="notchecked"  />
                    ...
                    <div id="mainent_form">
<?php
$adds_query = mysql_query("Select * from llproducts where home_prod_id = '1'");
	$adds_res = mysql_fetch_array($adds_query);
?>
                    <h1 id="toptitle"><?php echo $adds_res['subscribe_title']; ?><br /><span>Plus ALL NEW</span></h1>
                    <ul id="newoptions">
						<li class="nostyle">
							<a href="" class="loginimage" id="selectfree" style="display:none;"><h2>Free lalanii.com access (limited)</h2></a>
							
							<a href="" class="loginimagetoggles" id="selectfreetoggle"><h2 style="text-align:center;">Free lalanii.com access (limited)</h2></a>
                    ...
                    <ul id="newagreement">				
						...
						<?php if($_GET['get_product'] == true){ ?>
						<input type="hidden" tabindex="11" name="usertypers" id="usertypers" value="getproducts">
						<input type="hidden" tabindex="11" name="prodid" id="usertypers" value="<?php echo $_GET['prodid']; ?>">
						<?php }  else { ?>
						<input type="hidden" tabindex="11" name="usertypers" id="usertypers" value="downloadebook">
						<?php } ?>
					</ul>
...
$check_user = mysql_query("Select * from llUser where userID='$uid' ");
$result_user = @mysql_fetch_array($check_user);
$ebook_status = $result_user['ebook_status'];


				if($resultEbook['status'] == 0 )
				{
?><h2 style="font-size: 88px;margin: 10px 0px 0px 0px;">COMING SOON!</h2>
<?php } 
else if($ebook_status == 1){
?>

<h2>"You're backkkk... do you need another e-book? Please make sure the title is the latest e-book you want, k?"</h2>
<?php }else { 
	$adds_query = mysql_query("Select * from llproducts where home_prod_id = '".$_GET['prodid']."'");
	$adds_res = mysql_fetch_array($adds_query);
?>
	<h1 class="impactLabel bluegreen"><?php echo $adds_res['subscribe_title']; ?></h1>
	<?php echo $adds_res['subscribe_txt']; ?>
...
<table border="0" cellpadding="0" style="border-collapse: collapse">
		...
		<tr>
			<td>Email:</td>
			<td><input type="hidden" name="userID" size="20" value="<?php echo $userid; ?>">&nbsp;</td>
		</tr>
		<tr>
        ...
        <?php
			if($_GET['get_product'] == true)
		{
		?>
		<li>
<div style=""><input id="lasttype" type="hidden" value=""><input id="ebookcost" type="hidden" value="<?php echo $adds_res['prod_price']; ?>"><input onChange="caltotal(this.value);" type="checkbox" value="eproduct" id="egroup" name="subscriptione"><span id="spink" style="cursor:pointer;color:<?php echo $pdfresult['pheadingColor']; ?>;font-size:<?php echo $pdfresult['pheadingSize']; ?>;"><label style="cursor:pointer;" for="egroup"><?php echo $adds_res['prod_name']; echo '  $'.$adds_res['prod_price']; ?></span></label></div>
		<br />
		</li>
		<?php } else { ?>
		<li>
		...
		if(@mysql_num_rows($pdfquery) >= 1){
		$pdfresult = mysql_fetch_array($pdfquery);
		?>
        <div style=""><input id="lasttype" type="hidden" value=""><input id="ebookcost" type="hidden" value="<?php echo $pdfresult['ppdfamount']; ?>"><input onChange="caltotal(this.value);" type="checkbox" value="ebook" id="egroup" name="subscriptione"><span id="spink" style="cursor:pointer;color:<?php echo $pdfresult['pheadingColor']; ?>;font-size:<?php echo $pdfresult['pheadingSize']; ?>;"><label style="cursor:pointer;" for="egroup"><?php echo $pdfresult['pheading']; echo '  $'.$pdfresult['ppdfamount']; ?></span></label></div>
		<?php } ?><br />
        </li>
		<?php } ?>

        <li>Payment Method: <br />
        <input onClick="return paymth('paypalm')" type="radio" name="paypalm" value="paypalm" checked > Paypal<br />
        <!--<input onClick="return paymth('authorizem')" type="radio" name="paypalm" value="authorizem" checked > Authorize.Net<br /><br />-->
        </li>
		...
		<table border="0" cellspacing="0" cellpadding="0">
			...
			<tr>          
				<td>
					<input type="text" class="text required" required size="15" name="first_name" placeholder="first"><input type="text" class="text required" size="15" name="last_name" placeholder="last"></td>
			...
			<tr>
				<td>
					...
					-<input type="text" class="text required" size="4" name="exp_year" placeholder="yyyy">
				</td>
				<td></input><input type="text" class="text required" size="3" name="card_code" placeholder=""></td>
			</tr>
			<tr>
				<td>Address:</td>
				<td>&nbsp;</td>
          ...
          <input type="text" class="text required" size="5" name="zip" placeholder="zip" />
			...
			<tr>
				<td colspan="2"> 
				<input type="button" onClick="return paybtn('authorizem')" placeholder="register" class="redbg impactLabel" name="subscribe" value="subscribe"></td>
			</tr>
		</table>


		</li>
        <li  id="paypalm"><input type="button" onClick="return paybtn('paypalm')" placeholder="register" class="redbg impactLabel" name="subscribe" value="subscribe"><br></li>
	</ol>
</form>
	...
	<span id="siteseal"><script type="text/javascript" src="https://seal.godaddy.com/getSeal?sealID=1WnLyuqKnG2KLfiOzhryWlUnJUugvChOBkhfEoLZrZvrc5xVAkSAP9BvgCGo"></script></span>
	<?php } ?>
	</div>
	
	</div>
<?php include 'includes/scriptscwa.php';?>
  <?php
  }
  ?>
</div>

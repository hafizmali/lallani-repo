...
class rss_feed  {
   ...
   * @return array
   */
  public function get_feed_items($rss_date, $rss_items_count = 2) {
...
$dbname="lalanii";
    ...
    $a_topic_ids = array();
    $sql = "Select b.secretBlog,b.blogID,b.blogTitle,b.blogDetail,b.blogPage,b.blogDate,b.blogEmailed,b.blogThumb,b.blogBanner,s.categoryID,s.subcategoryID from llBlog b join llSubcategory s on b.subcategoryID=s.subcategoryID where b.post_to_fb ='1' ORDER BY b.blogDate desc  LIMIT 0," . $rss_items_count;
    ...
    while($res = $rs->fetch_assoc()) {
      array_push($a_topic_ids, $res['blogID']);
    }
    $rs->free();
    ...
    foreach($a_topic_ids as $topic_id) {
      ...
      $sql='SELECT * FROM llBlog WHERE blogID=' . $topic_id;
      $rs=$conn->query($sql);
      ...
      // title
      $a_rss_item['title'] = str_replace('&','&amp;', $topic['blogTitle']);
 
      // link
      $a_rss_item['link'] = 'http://lalanii.com/'.strtolower($topic['blogPage'])."/".$topic['blogID']."/".preg_replace('/[^A-Za-z0-9\-]/', '',  str_replace(' ','-',  $topic['blogTitle']));
 
      // description
      $a_rss_item['description'] = '';
 
      if($topic['blogBanner']) {
        $img_url = $topic['blogBanner'];
        $a_rss_item['description'] = '<img src="' . $img_url . '" hspace="5" vspace="5" align="left"/>';
      }
      $a_rss_item['description'] .= str_replace('&','&amp;', $topic['blogTitle']);
 
      // pubdate -> configure appropriately to your environment
      $date = new DateTime($topic["blogDate"]);
      $a_rss_item['pubDate'] = $date->format("D, d M Y H:i:s O");
 
      // category
      $a_rss_item['category'] = $topic['blogPage'];
 
      // source
      $a_rss_item['source'] = 'http://lalanii.com/';

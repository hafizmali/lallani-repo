...
<style>
#imglist li { height: 162px !important; }
</style>
</head>
<body id="imageditor">
<div id="main">
...
foreach ($_POST['imageID'] as $imageID) 
{
	...
	mysql_query("Update llbrandAdminimage set orderno='".$_POST['orderno_'.$imageID]."'  where imageID = '".$imageID."'");
//echo "Update llAdminimage set orderno='".$_POST['orderno']."' where imageID = '".$_POST['imageID']."'";
	...
	<div id="portalContent">
		...
		<div id="blogadmin">
		<h1 class="impactLabel redbg">Brand Administrative Image Editor</h1>
<?php $get_orderquery = mysql_fetch_array(mysql_query("Select max(orderno) as maxorder from llbrandAdminimage  ")); ?>
		<form action="http://lalanii.com/process/addbrandadminimage.php" method="post" enctype="multipart/form-data">
			Select image to upload:
			<input type="file" name="fileToUpload" id="fileToUpload">	
			Order No: <input type="text" readonly name="orderno" value="<?php echo $get_orderquery['maxorder']+1; ?>">		
            ...
            <form action="http://lalanii.com/includes/brandslidermanager.php" method="post" name="orderfrm<?php echo $IMG; ?>" >
			<ul id="imglist">
			<?php
			$sqlIMG="Select * from llbrandAdminimage order by orderno desc";
					...
					?>
					<li><img class="selectthisimage" src="http://lalanii.com/images/admin/<? echo $image; ?>" height="100px" /><br><?php echo $image; ?><br><a style="color:red;" href="http://lalanii.com/process/deletebrandadminimage.php?imageID=<?php echo $imageID; ?>" class="error">delete</a><br /><input style="width: 50px;text-align: center;" type="text" name="orderno_<?php echo $imageID; ?>" value="<?php echo $orderno; ?>"><input type="hidden" value="<?php echo $imageID; ?>" name="imageID[]"> <input type="submit" name="upd_btn" value="Update"></li>
					<?php
					++$IMG;
					}?>					
			</ul>

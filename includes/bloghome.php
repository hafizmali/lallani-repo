...
error_reporting(0);
	...
	$sqlBLOG="Select * from llBlog WHERE blogDate<=NOW() ORDER BY homepageBlog desc, blogDate desc, Month(blogDate) desc, Year(blogDate) desc";/*limit 10*/
	//echo "<br>SQL:".$sqlBLOG;
	...
	while ($BLOG < $throughrow) {
		...
		$blogDetailPart=substr(strip_tags($blogDetail),0,105)."...";
		...
		if($secretBlog == 1){$issecret="yes";}else{$issecret="nope";}
...
*/
	...
	?>
		...
		<a href="<?php echo $bloghomehref;?>">
        	<img class="displayedimage undefined" src="http://lalanii.com/image.php?width=194&height=176&image=<?php echo $blogThumb; ?>" width="200px" height="175px" style="display: inline;">
